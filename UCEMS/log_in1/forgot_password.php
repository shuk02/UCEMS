...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Check if email exists in the lecturers table
    $stmt = $con->prepare("SELECT username, email FROM lecturers WHERE email = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        // Update the lecturers table with the token, expiry, and status
        $stmt = $con->prepare("UPDATE lecturers SET token = ?, token_expiry = ?, token_status = ? WHERE email = ?");
        $stmt->bind_param("ssis", $token, $token_expiry, $token_status, $email);
        if ($stmt->execute()) {
            // Send email with reset link
            $reset_link = "http://localhost/UCEMS/log_in1/reset_password.php?token=" . $token;
                ...
                <div class="login_info">
                    ...
                    <form action="forgot_password.php" method="post">
                        ...
                        <div class="input-group">
                            ...
                            <input type="email" name="email" placeholder="Enter Your Lecturers Email" required>
                        </div>
